// extensions/models/swamp_club_extensions.ts
...
var model = {
  ...
  methods: {
    list: {
      description: "Fetch extensions published within the sinceDays window from swamp.club",
      arguments: z.object({}),
      execute: async (args, context) => {
        const { baseUrl, sinceDays } = context.globalArgs;
        const createdAfter = new Date(Date.now() - sinceDays * 24 * 60 * 60 * 1e3).toISOString();
        const browseDate = createdAfter.slice(0, 10);
        const browseUrl = `${baseUrl}/extensions?createdAfter=${browseDate}`;
        context.logger.info("Fetching extensions created after {since} from {baseUrl}", {
          since: createdAfter,
          baseUrl
        });
        var allExtensions = [];
        ...
        while (page <= totalPages) {
          ...
          if (!response.ok) {
            ...
            throw new Error(`Swamp Club API error (${response.status}): ${errorText}`);
          }
        ...
        const highlighted = allExtensions.filter((ext) => ext.description && ext.description.length > 10 && (ext.repository || ext.contentTypes && ext.contentTypes.length > 1)).slice(0, 5);
        ...
        const dataHandle = await context.writeResource("result", "result", result);
        return {
          dataHandles: [
            dataHandle
          ]
