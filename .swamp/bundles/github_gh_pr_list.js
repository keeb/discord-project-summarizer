// extensions/models/github_gh_pr_list.ts
      ...
      execute: async (args, context) => {
        ...
        const cmd = new Deno.Command("gh", {
          args: [
            ...
            String(limit),
            "--json",
            "number,title,url,mergedAt,author,body,closedAt"
          ],
          stdout: "piped",
          stderr: "piped"
        });
        ...
        const sinceDate = args.since ? new Date(args.since) : new Date(Date.now() - (args.sinceDays || 1) * 24 * 60 * 60 * 1e3);
        const pullRequests = prsRaw.filter((pr) => {
